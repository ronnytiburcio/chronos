...
Chronos/Window    DesktopPanel (NSPanel subclass), PanelController, FirstMouseHostingView, DragHandleView, PanelKeyWindow, WindowPlacement, SettingsWindowController
Chronos/Model     Project, Session, Settings, TrackingDay, ArchivedDay (+ ProjectTotal, ArchivedSession)
Chronos/Engine    TimerEngine, RolloverScheduler, Clock (injectable now())
Chronos/Storage   AppPaths (+ ArchiveLocation), JSONFile, AppStateStore (state.json), ProjectStore (projects.json), SessionLog (sessions.jsonl), CSV, ArchiveWriter, Exporter (+ ExportRange)
Chronos/UI        Palette, ProjectColor, TimeFormatting, PanelLayout, EscapeKeyMonitor,
                  PanelView (root) -> HeaderView / ProjectRowView (+ RowMenu) / FooterView, SettingsView, + BoltShape
...
- `CHRONOS_OPEN_SETTINGS=1` in the environment makes `AppDelegate` open the settings window at launch. Useful for screenshots: `CHRONOS_OPEN_SETTINGS=1 Chronos.app/Contents/MacOS/Chronos` (`open` does not forward the variable).
